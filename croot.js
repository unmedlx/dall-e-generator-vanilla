...
// Api Call
let target_url = `https://server-ai-img-generator.cyclic.app/api/v1/dalle`;
let tokenkey = "123";
let tokenvalue = "123";
let responseFunction = (res) => {
  if (res.photo) {
    let imgSrc = `data:image/jpeg;base64,${res.photo}`;
    let imgElement = document.createElement("img");
    imgElement.src = imgSrc;
    hide("imgPlaceholder");
    getId("image").appendChild(imgElement);
    hide("loader");
    show("submit");
  } else {
    hide("loader");
...
// Submit Handler
getQuery("form").addEventListener("submit", (e) => {
  hide("submit");
  show("loader");
  e.preventDefault();

  let promptInput = getId("prompt").value;
  let datajson = { prompt: promptInput };
  postWithToken(target_url, tokenkey, tokenvalue, datajson, responseFunction);
